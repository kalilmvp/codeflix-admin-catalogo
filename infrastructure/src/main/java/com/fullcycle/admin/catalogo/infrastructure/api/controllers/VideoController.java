...
public class VideoController implements VideoAPI {
    ...
    @Override
    public ResponseEntity<?> uploadMedia(String anId, String aType, MultipartFile aMediaFile) {
        final var aMediaType =
                VideoMediaType.of(aType).orElseThrow(() -> NotificationException.with(new Error("Invalid %s for VideoMediaType".formatted(aType))));

        final var aCmd = UploadMediaCommand.with(anId, VideoResource.with(this.resourceOf(aMediaFile), aMediaType));

        final var uploadResponse = VideoApiPresenter.present(this.uploadMediaUseCase.execute(aCmd));

        return ResponseEntity
                .created(URI.create("/videos/%s/media/%s".formatted(anId, aType)))
                .body(uploadResponse);
    }
